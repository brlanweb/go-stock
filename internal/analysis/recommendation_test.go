...
// 自动建仓能力已永久移除：即使遗留配置误设为 true，也只能推荐不能建仓。
func TestAutoEntryAlwaysDisabled(t *testing.T) {
	for _, config := range []Config{{}, {AutoEntryEnabled: true}} {
		if New(nil, config).AutoEntryEnabled() {
			t.Fatal("AI 自动建仓必须永久关闭")
		}
	}
}
