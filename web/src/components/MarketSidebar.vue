...
<template>
  <aside class="market-sidebar">
    ...
    <div v-if="syncStatus?.backfill_running" class="coverage-status"><span>历史完整 {{ syncStatus.backfill.complete }}/{{ syncStatus.backfill.total }}</span><small>部分 {{ syncStatus.backfill.partial }} · 空 {{ syncStatus.backfill.empty }}</small></div>

    <section class="sidebar-section recommendation-panel">
      <header><strong>AI 趋势推荐</strong><select v-if="recommendationDates.length" v-model="recommendationDate" title="查看历史推荐" @change="selectRecommendationDate"><option v-for="date in recommendationDates" :key="date" :value="date">{{ date }}</option></select></header>
...
.brand { padding:0 7px 7px; border-bottom:1px solid #354157; color:#f0f3f8; font-size:16px; font-weight:720; }.market-sidebar h1 { padding:0 7px; font-size:14px; font-weight:600; }
