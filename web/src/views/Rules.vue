    ...
    <div class="rules-layout">
      ...
      <article class="rules-document">
        ...
        <section id="funnel" class="rule-section">
          ...
          <div>
            ...
            <div class="formula-grid">
              ...
              <div><b>风险评分</b><span>年化波动率 40% + 60 日最大回撤 45% + 近 5 日过热 15%；只做展示，不参与过滤、排序或 AI 选举。</span></div>
        ...
        <section id="recommendation" class="rule-section">
          ...
          <div>
            ...
            <ul>
              ...
              <li>先按热点持续性与板块强度锁定最强板块，再从板块内选择成交活跃、趋势完整的龙头。</li>
              <li>优先 leader_rank 靠前且仍有合理建仓空间的龙头，保留既有涨停、高位过热与趋势结构规则。</li>
              <li>候选覆盖多个强板块时兼顾分散，3 只不得全部来自同一板块。</li>
              <li>风险分只随结果展示，不参与候选剔除、降权、排序或 AI 选举，由用户自行判断。</li>
              <li>前一日复盘只能调整趋势、板块与建仓空间判断，不能把风险分重新引入选举。</li>
            </ul>
          </div>
        </section>
        ...
        <section id="entry" class="rule-section">
          ...
          <div>
            <h2>自选与手动建仓</h2>
            <p>每日 3 只推荐固定展示在左侧，用户点击“+”后才加入自选。风险分仅展示；是否建仓由用户结合板块、龙头地位、建仓机会和风险分自行判断。</p>
            <div class="parameter-table">
              <div><span>每日推荐</span><b>3 只</b><small>强板块中的机会龙头</small></div>
              <div><span>自选容量</span><b>最多 10 只</b><small>点击加号加入、减号移除</small></div>
              <div><span>建仓金额</span><b>默认 100 万</b><small>余额不足时拒绝</small></div>
              <div><span>执行方式</span><b>用户手动确认</b><small>AI 只推荐不交易</small></div>
            </div>
            <p class="note">AI 推荐与每小时分析都不会改变资金和持仓。只有用户点击建仓或平仓时，系统才按当时行情生成模拟交易流水。</p>
          </div>
        </section>
        ...
        <section id="intraday" class="rule-section">
          ...
          <div>
            <h2>盘中分析时点与上下文</h2>
            <p>交易日每小时分析自选股：<strong>10:00、11:00、13:30、14:30</strong>。AI 只形成买入、卖出或持有建议并记录，不自动执行任何交易。</p>
