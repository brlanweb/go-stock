      ...
      <template v-else>
        ...
        <section class="panel sectors-panel">
          ...
          <div class="sector-table">
            ...
            <div v-for="item in report.sector_assessments" :key="item.sector_code" class="sector-row" @click="router.push(`/sector/${encodeURIComponent(item.sector_code)}`)">
...
.panel { padding:12px; }.sector-table { overflow-x:auto; }.sector-row { display:grid; grid-template-columns:120px 48px 65px 65px 70px 58px minmax(180px,1.4fr) minmax(160px,1fr); gap:10px; align-items:center; min-width:920px; padding:9px 8px; border-bottom:1px solid #202d43; color:#bdc6d4; font-size:11px; }.sector-row.head { color:#738097; }.sector-row:not(.head) { cursor:pointer; }.sector-row:not(.head):hover { background:#1b2740; }.sector-row span:first-child b,.sector-row span:first-child small { display:block; }.sector-row span:first-child b { color:#e7ecf4; font-size:12px; }.sector-row span:first-child small { margin-top:2px; color:#67748a; font-size:9px; }.strength { font-weight:700; }.strength.strong { color:#ef6a72; }.strength.weak { color:#46bd91; }.strength.neutral { color:#e9c16c; }
