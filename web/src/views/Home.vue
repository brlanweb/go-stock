...
import { api, fmtPct, pctClass, type EntryAdviceResponse, type Position, type Recommendation, type RecommendationBasketPerformance, type RecommendationStats } from '../api'
...
const basketChart = computed(() => {
  ...
  const width = 1000, height = 230, left = 52, right = 18, top = 18, bottom = 30
  const plotW = width - left - right, plotH = height - top - bottom
  const maxAbs = Math.max(1, ...values.map(item => Math.abs(item.value)))
  const x = (index: number) => left + (index + 0.5) * plotW / Math.max(values.length, 1)
  const y = (value: number) => top + (maxAbs - value) / (maxAbs * 2) * plotH
  const barWidth = Math.max(8, Math.min(34, plotW / Math.max(values.length, 1) * 0.62))
  const labels = values.filter((_, index) => index === 0 || index === values.length - 1 || index % Math.max(1, Math.ceil(values.length / 8)) === 0)
  ...
  return { width, height, left, right, values, maxAbs, zeroY: y(0), x, y, barWidth, labels, samples, frozen, tracking }
})
    ...
    <template v-else>
      ...
      <section class="dashboard-grid">
        <article class="panel basket-panel">
          <header><div><b>每日三只趋势推荐组合</b><small>每个推荐日等权汇总 3 只；次日开盘为参考起点，规则退出后冻结，未退出跟随最新收盘</small></div><strong>{{ basketChart.values.length }} 日 · {{ basketChart.samples }} 只</strong></header>
          <div v-if="basketChart.values.length" class="chart-wrap basket-chart"><svg :viewBox="`0 0 ${basketChart.width} ${basketChart.height}`" role="img" aria-label="每日三只趋势推荐等权平均表现">
            <line :x1="basketChart.left" :x2="basketChart.width - basketChart.right" :y1="basketChart.zeroY" :y2="basketChart.zeroY" class="zero"/>
            <rect v-for="(point,index) in basketChart.values" :key="point.date" :x="basketChart.x(index)-basketChart.barWidth/2" :y="Math.min(basketChart.y(point.value),basketChart.zeroY)" :width="basketChart.barWidth" :height="Math.max(2,Math.abs(basketChart.zeroY-basketChart.y(point.value)))" :class="['basket-bar',point.value>=0?'gain':'loss']"><title>{{ point.date }}：{{ point.stocks }} 只等权平均 {{ signed(point.value) }}，合计 {{ signedPoints(point.sum_change_pct) }}；已冻结 {{ point.frozen_stocks }}，跟踪中 {{ point.tracking_stocks }}</title></rect>
            <text :x="basketChart.left-8" :y="basketChart.y(basketChart.maxAbs)+4" class="axis y">{{ basketChart.maxAbs.toFixed(1) }}</text><text :x="basketChart.left-8" :y="basketChart.zeroY+4" class="axis y">0</text><text :x="basketChart.left-8" :y="basketChart.y(-basketChart.maxAbs)+4" class="axis y">{{ (-basketChart.maxAbs).toFixed(1) }}</text>
            <text v-for="label in basketChart.labels" :key="label.date" :x="basketChart.x(basketChart.values.indexOf(label))" :y="basketChart.height-8" class="axis x">{{ label.date.slice(5) }}</text>
          </svg></div>
          <footer><span>红柱：当日三只平均上涨</span><span>绿柱：当日三只平均下跌</span><strong>已冻结 {{ basketChart.frozen }} · 跟踪中 {{ basketChart.tracking }}</strong></footer>
        </article>
