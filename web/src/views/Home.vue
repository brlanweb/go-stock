  ...
  <main class="overview">
    ...
    <template v-else>
      <section class="metric-band">
        <article class="metric primary"><small>已退出胜率</small><b :class="(stats?.win_rate ?? 0) >= 50 ? 'up' : 'down'">{{ stats?.win_rate == null ? '—' : `${stats.win_rate.toFixed(1)}%` }}</b><span>{{ stats?.wins || 0 }} 胜 / {{ stats?.losses || 0 }} 负 / {{ stats?.breakeven || 0 }} 平</span></article>
