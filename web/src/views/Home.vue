...
const basketChart = computed(() => {
  ...
  const width = 1600, height = 260, left = 48, right = 12, top = 24, bottom = 32
  ...
  return { width, height, left, right, values, min, max, zeroY: y(0), x, y, points, labels, samples, frozen, tracking }
})
      ...
      <section class="dashboard-grid">
        <article class="panel basket-panel">
          ...
          <div v-if="basketChart.values.length" class="chart-wrap basket-chart"><svg :viewBox="`0 0 ${basketChart.width} ${basketChart.height}`" role="img" aria-label="每日三只趋势推荐等权平均表现">
            ...
            <polyline :points="basketChart.points" class="basket-line"/>
            <circle v-for="(point,index) in basketChart.values" :key="point.date" :cx="basketChart.x(index)" :cy="basketChart.y(point.value)" r="4" :class="['basket-point',point.value>=0?'gain':'loss']"><title>{{ point.date }}：{{ point.stocks }} 只等权平均 {{ signed(point.value) }}，合计 {{ signedPoints(point.sum_change_pct) }}；已冻结 {{ point.frozen_stocks }}，跟踪中 {{ point.tracking_stocks }}</title></circle>
            <text :x="basketChart.left-8" :y="basketChart.y(basketChart.max)+4" class="axis y">{{ basketChart.max.toFixed(1) }}</text><text :x="basketChart.left-8" :y="basketChart.zeroY+4" class="axis y">0</text><text :x="basketChart.left-8" :y="basketChart.y(basketChart.min)+4" class="axis y">{{ basketChart.min.toFixed(1) }}</text>
          ...
          <footer><span>折线：每日三只等权平均涨跌幅</span><strong>已冻结 {{ basketChart.frozen }} · 跟踪中 {{ basketChart.tracking }}</strong></footer>
        </article>
