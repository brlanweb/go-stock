...
import { api, fmtBig, fmtPct, type HeatmapGroup, type HeatmapItem, type Quote, type Recommendation, type SyncStatus } from '../api'
...
async function refreshSidebar() {
  const [recommendationResult, watchlistResult, syncResult] = await Promise.allSettled([api.recommendations(), api.watchlist(), api.syncStatus()])
...
.coverage-status { display:grid; gap:2px; padding:5px 7px; border-left:2px solid #d6a12c; background:#2b2c32; color:#d9e0e9; font-size:10px; }.coverage-status small { color:#9ba8bd; font-size:9px; }
